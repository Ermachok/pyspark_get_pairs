...
def get_product_category_pairs(
    ...
    result = (
        products.join(product_category, on="product_id", how="inner")
...
def get_products_without_categories(
    products: DataFrame, product_category: DataFrame
) -> DataFrame:
    logger.info("Ищем продукты без категории...")

    result = (
        products.join(product_category, on="product_id", how="left")
